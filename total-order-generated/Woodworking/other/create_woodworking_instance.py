...
class Part(object):
    ...
    def generate_random_goal(self, woods, colours):
        ...
        self.goalprops["surface_condition"] = random.choice(["smooth", "verysmooth"])
    ...
    def generate_random_init(self, woods, colours):
        def gen_preprocessing_status():
            ...
            self.initprops["surface_condition"] = random.choice(
    ...
    def dump_htn_param(self, i, indent="", out=None):
        print("%s?v_%d_1 - acolour ?v_%d_2 - surface ?v_%d_3 - surface" % (indent,i,i,i), file=out)
   
    def dump_htn(self, i, indent="", out=None):
        print("%s(task%s (process %s ?v_%d_1 ?v_%d_2 ?v_%d_3))" % (indent, self.name, self.name,i,i,i), file=out)
...
class Board(object):
    ...
    def dump_init(self, indent="", out=None):
        ...
        print(
            "%s(surface_condition %s %s)" % (indent, self.name, self.surface), file=out
        )
        print("%s(available %s)" % (indent, self.name), file=out)
...
class Machine(object):
    ...
    def dump_init(self, indent, out=None):
        if self.type == "highspeed-saw":
            ...
            print("%s(machine_present %s)" % (indent, self.name), file=out)

        for colour in self.colours:
            print("%s(has_colour %s %s)" % (indent, self.name, colour), file=out)
...
class Task(object):
    ...
    def _assign_colours_to_machines(self):
        ...
        spray_varnishers = self.machines.get("spray_varnisher", [])
        immersion_varnishers = self.machines.get("immersion_varnisher", [])
        ...
        for type in ["spray_varnisher", "immersion_varnisher", "glazer"]:
    ...
    def _dump_htn(self, indent="", out=None):
        print(indent + "(:htn", file=out)

        print(indent + "  :parameters (", file=out)
        i=1
        for part in self.parts:
            part.dump_htn_param(i, indent + "    ", out)
            i = i +1
        print(indent + "  )", file=out)
        
        print(indent + "  :ordered-subtasks (and", file=out)
        i=1
        for part in self.parts:
            part.dump_htn(i, indent + "    ", out)
            i = i +1
    ...
    def _dump_init(self, indent="", out=None):
        ...
        print(indent + "  (grind_treatment_change varnished colourfragments)", file=out)
        print(indent + "  (grind_treatment_change glazed untreated)", file=out)
        print(indent + "  (grind_treatment_change untreated untreated)", file=out)
        print(indent + "  (grind_treatment_change colourfragments untreated)", file=out)
        print(indent + "  (is_smooth smooth)", file=out)
        print(indent + "  (is_smooth verysmooth)", file=out)
        #print(indent + "  (= (total-cost) 0)", file=out)

        for size in range(self.max_board_size):
            print(
                "%s  (boardsize_successor %s%d %s%d)"
