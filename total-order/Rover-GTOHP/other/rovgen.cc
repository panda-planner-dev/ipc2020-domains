...
public:
		...
		if(!accessible_for_soil.empty()){
			for(int i = 0;i<numsoilgoals;){ 
					...
					if(!waypoints[s].visited_s){
					  ...
					  Goal hg("(task" + std::to_string(i) + " (get_soil_data waypoint",s,"))");
					  waypoints[s].visited_s = true;
					  thegoals.push_back(g);
					  theHTNgoals.push_back(hg);
					  ++i;
					}
		...
		if(!accessible_for_rock.empty()){
			for(int i = 0;i<numrockgoals;){
					...
					if(!waypoints[*s].visited_r){
						...
						Goal hg("(task" + std::to_string(i + numsoilgoals) + " (get_rock_data waypoint",*s,"))");
				...
				switch(mode)
				{
					case 1:
					{
						if(find(objectives[w].requests.begin(),objectives[w].requests.end(),1)==objectives[w].requests.end()){
							...
							Goal hg("(task" + std::to_string(i + numrockgoals + numsoilgoals) + " (get_image_data objective",w," high_res))");
					...
					case 2:
					{
						if(find(objectives[w].requests.begin(),objectives[w].requests.end(),2)==objectives[w].requests.end()){
							...
							Goal hg("(task" + std::to_string(i + numrockgoals + numsoilgoals) + " (get_image_data objective",w," low_res))");
					...
					default:
					{
						if(find(objectives[w].requests.begin(),objectives[w].requests.end(),3)==objectives[w].requests.end()){
							...
							Goal hg("(task" + std::to_string(i + numrockgoals + numsoilgoals) + " (get_image_data objective",w," colour))");
	...
	void write(ostream & o) const
	{
		...
		copy(theHTNgoals.begin(),theHTNgoals.end(),ostream_iterator<Goal>(o,"\n"));
