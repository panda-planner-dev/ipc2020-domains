(defproblem problem domain 
  (
    (In_City O27 Ulm)
    (In_City O28 Ulm)
    (At_Vehicle Pferd O27)
    (Empty CraneO27)
    (Available CraneO27)
    (At_Equipment CraneO27 O27)
    (Empty CraneO28)
    (Available CraneO28)
    (At_Equipment CraneO28 O28)
    (Connects James_Franck_Ring O27 O28)
    (Available James_Franck_Ring)
    (Available Pferd)
    (PV_Compatible Holz Pferd)
    (RV_Compatible James_Franck_Ring Pferd)
    (At_Package Holz O27)
    (type_Bulky Holz)
    (type_City Ulm)
    (type_City_Location O27)
    (type_City_Location O28)
    (type_Crane CraneO27)
    (type_Crane CraneO28)
    (type_Customer_Location O27)
    (type_Customer_Location O28)
    (type_Equipment CraneO27)
    (type_Equipment CraneO28)
    (type_Equipment_Position O27)
    (type_Equipment_Position O28)
    (type_Equipment_Position Pferd)
    (type_Equipment_Position Ulm)
    (type_Flatbed Pferd)
    (type_Flatbed_Truck Pferd)
    (type_Flatbed_Vehicle Pferd)
    (type_Local_Road_Route James_Franck_Ring)
    (type_Location O27)
    (type_Location O28)
    (type_Location Ulm)
    (type_Lumber Holz)
    (type_Not_TCenter O27)
    (type_Not_TCenter O28)
    (type_Object CraneO27)
    (type_Object CraneO28)
    (type_Object Holz)
    (type_Object Pferd)
    (type_Package Holz)
    (type_Package_Storage_Position CraneO27)
    (type_Package_Storage_Position CraneO28)
    (type_Package_Storage_Position O27)
    (type_Package_Storage_Position O28)
    (type_Package_Storage_Position Pferd)
    (type_Package_Storage_Position Ulm)
    (type_Physical Holz)
    (type_Physical Pferd)
    (type_Road_Route James_Franck_Ring)
    (type_Route James_Franck_Ring)
    (type_Thing CraneO27)
    (type_Thing CraneO28)
    (type_Thing Holz)
    (type_Thing James_Franck_Ring)
    (type_Thing O27)
    (type_Thing O28)
    (type_Thing Pferd)
    (type_Thing Ulm)
    (type_Truck Pferd)
    (type_Vehicle Pferd)
    (type_Vehicle_Position O27)
    (type_Vehicle_Position O28)
    (type_Vehicle_Position Ulm)
    (type_sort_for_CraneO27 CraneO27)
    (type_sort_for_CraneO28 CraneO28)
    (type_sort_for_Holz Holz)
    (type_sort_for_James_Franck_Ring James_Franck_Ring)
    (type_sort_for_O27 O27)
    (type_sort_for_O28 O28)
    (type_sort_for_Pferd Pferd)
    (type_sort_for_Ulm Ulm)
  )
  ((__top))
)
