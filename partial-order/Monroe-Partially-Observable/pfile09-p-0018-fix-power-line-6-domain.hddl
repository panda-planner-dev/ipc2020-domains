...
(:method m_tlt_set_up_shelter
	...
	(:method m_repair_line_with_tree
		...
		:subtasks (and
		 ...
		 (task1 (clear_tree ?tree))
		 (task2 (cremove_wire ?crew ?lineloc))
	...
	(:method m_repair_line_without_tree
		...
		:subtasks (and
		 (task0 (shut_off_power ?crew ?lineloc))
		 (task1 (cremove_wire ?crew ?lineloc))
	...
	(:method regular_2
		:parameters (?crew - power_crew ?lineloc - point)
		:task (cremove_wire ?crew ?lineloc)
		:subtasks (and
		 (task0 (remove_wire ?crew ?lineloc))
		)
	)
	(:method regular_3
		:parameters (?obj - person ?objloc - point ?veh - vehicle)
	...
	(:method prefix_3
		...
		:precondition (and
			(= ?crew pcrew1)
			(= ?lineloc brighton_high)
		)
		:subtasks (and
		 (task0 (p_3Remove_wire))
		)
	)
	(:method prefix_4
		:parameters (?loc - point ?person - adult ?veh - vehicle ?vehloc - point)
...
()
	...
	(:action p_1Navegate_vehicle
		...
		:effect
			(and
				(atloc van1 brighton_high)
				(atloc pcrew1 brighton_high)
	...
	(:action p_4Navegate_vehicle
		...
		:effect
			(and
				(atloc pvan2 airport)
				(atloc pu1 airport)
				(not (atloc pvan2 rochester_general))
	...
	(:action p_6Navegate_vehicle
		...
		:effect
			(and
				...
				(atloc pu1 brighton_high)
				(not (atloc pvan2 airport))
				(not (atloc pu1 airport))
				(l7)
				(not (l6))
			)
	)
